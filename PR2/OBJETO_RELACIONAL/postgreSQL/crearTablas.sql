...
-- 2. La fecha de nacimiento de un cliente, de creacion de una cuenta y la fecha y hora de una operacion, 
--    deben ser anteriores a la fecha actual.
CREATE OR REPLACE FUNCTION validar_fechas_generico() RETURNS TRIGGER AS $$
BEGIN
    -- Validar fecha de nacimiento en Cliente
    IF TG_TABLE_NAME = 'Cliente' AND NEW.fechaDeNacimiento >= CURRENT_DATE THEN
        RAISE EXCEPTION 'Error: La fecha de nacimiento debe ser anterior a la fecha actual.';
    
    -- Validar fecha de creación en Cuenta
    ELSIF TG_TABLE_NAME = 'Cuenta' AND NEW.fechaDeCreacion >= CURRENT_DATE THEN
        RAISE EXCEPTION 'Error: La fecha de creación de la cuenta debe ser anterior a la fecha actual.';

    -- Validar fecha de operación en Operacion
    ELSIF TG_TABLE_NAME = 'Operacion' AND NEW.fechaYHora >= NOW() THEN
...
-- Aplicar el trigger a Cliente, Cuenta y Operacion
CREATE TRIGGER validar_fechas_cliente
BEFORE INSERT OR UPDATE ON Cliente
FOR EACH ROW EXECUTE FUNCTION validar_fechas_generico();

CREATE TRIGGER validar_fechas_cuenta
BEFORE INSERT OR UPDATE ON Cuenta
FOR EACH ROW EXECUTE FUNCTION validar_fechas_generico();

CREATE TRIGGER validar_fechas_cuenta_corriente
BEFORE INSERT OR UPDATE ON CuentaCorriente
FOR EACH ROW EXECUTE FUNCTION validar_fechas_generico();

CREATE TRIGGER validar_fechas_cuenta_ahorro
BEFORE INSERT OR UPDATE ON CuentaAhorro
FOR EACH ROW EXECUTE FUNCTION validar_fechas_generico();

CREATE TRIGGER validar_fechas_operacion
BEFORE INSERT OR UPDATE ON Operacion
FOR EACH ROW EXECUTE FUNCTION validar_fechas_generico();

CREATE TRIGGER validar_fechas_operacion_efectiva
BEFORE INSERT OR UPDATE ON OperacionEfectiva
FOR EACH ROW EXECUTE FUNCTION validar_fechas_generico();

CREATE TRIGGER validar_fechas_transferencia
...
BEGIN
    -- Verificar si la cuenta ya existe en la tabla padre
