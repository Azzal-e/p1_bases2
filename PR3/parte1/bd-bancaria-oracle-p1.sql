...
BEGIN
    -- Obtener fecha de creación de la cuenta emisora
    ...
    -- Verificar fecha de la cuenta receptora solo si es TRANSFERENCIA
    ...
    ELSE
        v_fecha_creacion_Receptora := NULL; -- Asegurar que es NULL
    END IF;

    -- Validar fechas (solo compara con la receptora si no es NULL)
    IF :NEW.fechaOperacion <= v_fecha_creacion_Emisora 
       OR (:NEW.tipoOperacion = 'TRANSFERENCIA' AND :NEW.fechaOperacion <= v_fecha_creacion_Receptora) THEN
