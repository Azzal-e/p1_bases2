...
SELECT 
    DNI, 
    nombre, 
    COALESCE(SUBSTR(apellidos, 1, INSTR(apellidos, ' ') - 1), 'Desconocido') AS apellido1, 
    COALESCE(SUBSTR(apellidos, INSTR(apellidos, ' ') + 1), 'Desconocido') AS apellido2, 
    COALESCE(direccion, 'Dirección no disponible') AS direccion, 
    telefono, 
    fechaNacimiento
FROM Clientes
UNION
...
SELECT 
    ...
    saldo,
    CASE 
      WHEN esCuentaCorriente = 1 
        THEN 'CORRIENTE' 
      ELSE 
        'AHORRO' 
    END AS tipoCuenta,
    interes, 
    codigoOficina_Adscrita AS codigoOficina
FROM Cuentas
UNION
SELECT 
    C.CCC AS iban, 
    ...
    C.SALDO,
    CASE 
      WHEN CC.CCC IS NOT NULL 
        THEN 'CORRIENTE' 
      ELSE 
        'AHORRO' 
    END AS tipoCuenta,
    A.TIPOINTERES AS interes, 
    CC.SUCURSAL_CODOFICINA AS codigoOficina
...
SELECT
    ...
    NVL(TRIM(t.CUENTADESTINO), 'N/A') AS cuentaReceptora, 
